...
export function CertificationsSection() {
    ...
    return (
        <div className="mx-auto max-w-5xl px-8 pb-8">
            <h1 id="certifications" className="max-w-5xl pt-20 text-2xl font-bold dark:text-white md:pt-32 md:text-7xl">
                Certifications
            </h1>
            <ul className="mt-8 space-y-6">
                {certifications.map((cert, idx) => (
                    <li key={idx} className="flex items-center space-x-4 rounded-lg border border-black/[0.2] p-4 dark:border-white/[0.2]">
                        <Image src={cert.logo} alt={`${cert.provider} logo`} width={40} height={40} className="object-contain" />
                        <div>
                            <a
                                href={cert.link}
                                target="_blank"
                            ...
                            <div className="text-sm text-neutral-700 dark:text-neutral-300">{cert.provider}</div>
                        </div>
                    </li>
                ))}
            </ul>
